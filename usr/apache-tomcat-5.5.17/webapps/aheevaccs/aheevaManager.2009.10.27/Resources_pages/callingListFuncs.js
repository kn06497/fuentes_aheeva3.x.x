...
function saveCallingList(withoutFilter){
   ...
   if(document.callingList.table_name.value.length == 0)  {
		alert(MultiArray["You must enter the table name associated with this list"][lang]);
...
function askDelete()
{
	var answer = confirm(MultiArray["Are you sure, you wish to delete this calling list?"][lang]);
	if(answer)
	{
		...
		if(isLoaded == "1")
			alert(MultiArray["Unable to delete a calling list associated to a running campaign"][lang]);
		else
		{
	...
	switch(current_step)
	{
		case "1":
			...
			if(!isAlphaNumeric(listName)){
				alert(MultiArray["Invalid list name"][lang]);
			} else if(!isAlphaNumeric(call_list)){			
				alert(MultiArray["The name of the associated table to the calling list contains one or more forbidden characters. A calling list must start with a letter and contain only letters, digits and the underscore character(_). Please try again"][lang]);
		...
		case "5": //validate the after queries mega form
			...
			foundone = atleastonechecked(theForm.timeZoneRulesApplyto);
			if(!foundone)
			{	alert(MultiArray["Error: Missing Time Zone Rules Choice"][lang]); 	
			...
			else
			{
				...
				if(theForm.timezone_selectedrules.options.length ==0 && theForm.timeZoneRulesApplyto[2].checked == false ){
					...
					alert(MultiArray["Error: No rule were selected in Time Zone Setup. Try using the \'None\' option instead."][lang]); 
					goNext=false;
				}
				else{
...
function saveCallingListSettings(formatName)
{
	...
	{
		alert(MultiArray["Specify format name"][lang]); 
	}	
}

...
function CallEnding()
{
	...
	{			
		...
		sReturn += '<tr bgcolor="#F5CDA0"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDCallEnding_'+this.uID+'\')"><b>'+MultiArray["End"][lang]+' #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'CE\', \''+this.VUID+'\')"><i>'+MultiArray["Entry point"][lang]+'</i></td></tr>';
	...
	this.validate = function()
	{
		...
		if(this.action == "")
		{
			alert(MultiArray["Error: Missing variable name for call ending #"][lang]+this.uID);

...
function validateRanges(from, to, pay){
	...
	if (isNaN(parseFloat(from)) || isNaN(parseFloat(to)) || isNaN(parseFloat(pay))){
		alert(MultiArray["MOP Fill Fields"][lang]);
		return false;
	}
	...
	if (max > 0 && parseFloat(pay) > max){
		alert(MultiArray["MOP Error Max"][lang]);
		return false;
	}
	return true;
}
...
function readFixedValue(){
	...
	if (isNaN(parseFloat(document.mop_treatment_form.amount.value))){
		alert(MultiArray["MOP Error Negative"][lang]);
		return false;				
	}
...
function validateInfo()
{
	...
	else if(validation(document.mop_treatment_form.mopTreatmentDescription.value)==0)//checks Mop description
	{
		...
		alert(MultiArray["MOP Error Max Negative"][lang]);
	}
	...
	{             
		...
		for (var i = 0; i < arrayMopCommissionsNames.length; i++){
			...
			{
				alert(MultiArray["MOP Change Name"][lang]);
				return false;
			}
		}
...
function askDelete()
{
	...
	answer = confirm(MultiArray["Are you sure you wish to delete this commission?"][lang]);
...
function addMyRow(var1, var2, var3)
{
	...
	row.id = "row" + lineNum;
	row.innerHTML = "From: <input name='from' type='text' size='4' value=" + var1 + ">&nbsp; &nbsp; To: <input name='to' type='text' size='4' value=" + var2 + ">&nbsp; &nbsp; Pay: <input name='pay' type='text' size='4' value=" + var3 + "> &nbsp; &nbsp;<a class='delete' href='javascript:deleteMyRow("+lineNum+");'>"+MultiArray["Delete"][lang]+"</font></a></td></TR>";

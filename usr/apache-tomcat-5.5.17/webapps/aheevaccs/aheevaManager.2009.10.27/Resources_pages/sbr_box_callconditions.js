...
function CallConditionTest()
{
	...
	this.buildHTML = function()
	{
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'CC\', \''+this.VUID+'\')"><i>'+MultiArray["Entry point"][lang]+'</i></td></tr>';
		...
		sReturn += '';
		sReturn += '<tr><td colspan="2" nowrap class="onsuccedTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onSucced\', \'CC\', \''+this.VUID+'\')"><i>'+MultiArray["On success"][lang]+'';
		if(DEBUG)
			sReturn += ' -->['+this.OnSuccessVUID+']';
		sReturn += '</i></td></tr>';
		sReturn += '<tr><td colspan="2" nowrap class="onfailTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onFailed\', \'CC\', \''+this.VUID+'\')"><i>'+MultiArray["On failed"][lang]+'';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		{
			alert(MultiArray["Error: Missing test for call test condition #"][lang]+this.uID);
		...
		{
			alert(MultiArray["Error: Missing link type for call test condition #"][lang]+this.uID);
...
/*			alert(MultiArray["Specify tenant ID"][lang]);*/
		...
		{
			alert(MultiArray["Error: Missing OnSuccess action for call test condition #"][lang]+this.uID);

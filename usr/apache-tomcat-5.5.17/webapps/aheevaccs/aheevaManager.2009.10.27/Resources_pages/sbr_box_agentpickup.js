...
function agentPickUpMethod()
{
	...
	this.vectorAssignedVariables	= new Array();
	...
	{	
		...
		sReturn += '<tr bgcolor="#C5D3EB"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDPickMethod_'+this.uID+'\')"><b>'+MultiArray["Pick Up Method"][lang]+' #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'PM\', \''+this.VUID+'\')"><i>'+MultiArray["Entry point"][lang]+'</i></td></tr>';
		...
		sReturn += '<tr><td colspan="2" nowrap class="onsuccedTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onTimeOut\', \'PM\', \''+this.VUID+'\')"><i>'+MultiArray["On timeout"][lang]+'';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		if(this.havingClause == "")
			{
			alert(MultiArray["Error: Missing minimum required for agent pickup method #"][lang]+this.uID);
			...
			{
			alert(MultiArray["Error: Missing tiebreaker for agent pickup method #"][lang]+this.uID);
			...
			{
			alert(MultiArray["Error: Missing call priority for agent pickup method #"][lang]+this.uID);
			...
			{
			alert(MultiArray["Error: Missing virtual queue name for agent pickup method #"][lang]+this.uID);
			...
			{
			alert(MultiArray["Error:Virtual queue name for agent pickup method #"][lang]+this.uID+ MultiArray["cannot be the routing script name"][lang]);
...
function submitSetUpPM()
{
	...
	for(i = 0; i < vectorAgentPickupMethods.length; i++)
	{
		...
		{			
			...
			vectorAgentPickupMethods[i].vectorAssignedVariables = new Array();
			var assignedVariableObj = document.getElementById("PMSetupDialog_AssignedVars");
			for(var j = 0; j < assignedVariableObj.length; j++)
			{
				vectorAgentPickupMethods[i].vectorAssignedVariables.push(assignedVariableObj[j].value);
			}
			break;
		}
	}
				 ...
				 p_voicemess_you_are, p_voicemess_holdtime, p_voicemess_minutes, p_voicemess_thanksyouforyourpatience)
{
	...
	*/
	...
	var availableVariableObj = document.getElementById("PMSetupDialog_AvailableVars");
	var assignedVariableObj = document.getElementById("PMSetupDialog_AssignedVars");
	if(availableVariableObj && assignedVariableObj)
	{
		availableVariableObj.length = 0;
		assignedVariableObj.length = 0;
		//Get the rigth Object
		var assignedVariables = null;
		for(var i = 0; i < vectorAgentPickupMethods.length; i++)
		{		
			if(vectorAgentPickupMethods[i].uID == p_uID )
			{			
				assignedVariables = vectorAgentPickupMethods[i].vectorAssignedVariables;
				break;
			}
		}
		if(assignedVariables != null)
		{
			for(var i = 0; i < assignedVariables.length; i++)
			{
				var curOption = new Option(assignedVariables[i], assignedVariables[i]);
				assignedVariableObj.options[i] = curOption;
			}
			for(var i = 0; i < vectorOfCallProperties.length; i++)
			{
				var curOption = vectorOfCallProperties[i];
				if(!selectContainsValue("PMSetupDialog_AssignedVars", curOption.value))
					availableVariableObj.options[availableVariableObj.length] = curOption;
			}
		}
	}
	...
	PMSetupDialogObj.style.top = document.body.scrollTop + 50;
...
function isValideExpression(expr, uid)
{
	...
	{		
		...
		if(nbParen == -1)
		{
			...
			alert(MultiArray["Error: Unexpected token in agent pickup method #"][lang]+uid+ MultiArray[" having clause.\n Near: "][lang]+subExpr+MultiArray["\n Char:"][lang]+j);
	...
	{
		alert(MultiArray["Error: Too much open brackets in agent pickup method #"][lang]+this.uID);

...
{
	if(document.campaign.load.value == MultiArray["Load"][lang]){
		if(document.campaign.dialingMode.value == 'BROADCAST' && document.campaign.routing_script.value == ""){
			...
			document.campaign.status.value 		= MultiArray["Loaded"][lang];
			document.campaign.start.disabled 	= false;
			document.campaign.load.value  		= MultiArray["Unload"][lang];
			loadCampaign();
		}
	}else{
		...
		if(answer){
			document.campaign.load.value  		= MultiArray["Load"][lang];
			document.campaign.status.value 		= MultiArray["Not loaded"][lang];
			document.campaign.start.value 		= MultiArray["Start"][lang];
...
{
	if(document.campaign.start.value == MultiArray["Start"][lang])
	{
		...
		{
			document.campaign.status.value		   			= MultiArray["Campaign Started"][lang];
			document.campaign.start.value					= MultiArray["Stop"][lang];
	...
	{
		document.campaign.status.value					= MultiArray["Campaign Stopped"][lang];
		document.campaign.start.value					= MultiArray["Start"][lang];
//		document.campaign.OptimizationParameter.disabled 	= false;
		document.campaign.dialingMode.disabled				= false;
		setDialingMode();
...
function showCampaignCurrentStatus()
{
	...
	if(status.length != 0)
	{
		...
		{
			document.campaign.load.value  						= MultiArray["Unload"][lang];
			document.campaign.status.value 						= MultiArray["Campaign Loaded"][lang];
			document.campaign.start.value 						= MultiArray["Start"][lang];
			document.campaign.start.disabled 					= false;
			document.campaign.dialingMode.disabled 				= false;
			document.campaign.OptimizationParameter.disabled 	= false;
		...
		{
			document.campaign.load.value						= MultiArray["Unload"][lang];
			document.campaign.status.value						= MultiArray["Campaign Running"][lang];;
			document.campaign.start.value						= MultiArray["Stop"][lang];
			document.campaign.start.disabled    				= false;
			document.campaign.dialingMode.disabled 				= true;
			document.campaign.OptimizationParameter.disabled 	= true;
...
function refresh_field(callingListNames, group_list)
{
	...
	xhr.onreadystatechange = function(){
		...
		if(xhr.readyState == 4 && xhr.status == 200)
		{
			...
			hidePleaseWait();
			alert(MultiArray["All calling lists have been updated from Do-not-call lists"][lang] + MultiArray["Click OK to continue"][lang]); 
		}
	}
...
function validateInfo(){
	...
	if( (state == '0') && (document.campaign.load.value == MultiArray["Unload"][lang]) ){
		alert(MultiArray["Campaign cannot be deactivated while it is loaded."][lang]);
		document.campaign.cmpState.checked = true; 
		return;
	}

	if(document.campaign.status.value == MultiArray["Campaign Started"][lang] || document.campaign.status.value == MultiArray["Campaign Running"][lang]){
		var nb_total_configured_ports_campaign = appletObj.getTotalConfiguredPortsCampaign(cmpDBID);
		if((numberOfPortsAvailable - nb_total_configured_ports_campaign) - nb_ports < 0){
			var yesno = confirm(MultiArray["The total number of ports of started campaigns is higher"][lang] + "\n" + MultiArray["than the total configured number of ports."][lang] + "\n" + MultiArray["Would you like to continue anyway?"][lang]);
			if(!yesno)
...
function askDelete()
{
	if((document.campaign.load.value  == MultiArray["Unload"][lang]) | (document.campaign.start.value == MultiArray["Stop"][lang]))
	{

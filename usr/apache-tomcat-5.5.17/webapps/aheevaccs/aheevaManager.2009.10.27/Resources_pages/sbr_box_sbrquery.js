...
function SqlQuery()
{
	...
	{	
		...
		sReturn += '<tr bgcolor="#DECAB6"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDSqlQuery_'+this.uID+'\')"><b>'+MultiArray["Database query"][lang]+' #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'SQ\', \''+this.VUID+'\')"><i>'+MultiArray["Entry point"][lang]+'</i></td></tr>';
		sReturn += '<tr><td colspan="2"><input type="button" onClick="setUpSQ(\''+this.uID+'\')" ';
		sReturn += 'value="'+buttonValue+'" class="setupButton"></td></tr>';
		sReturn += '<tr><td colspan="2" nowrap class="onsuccedTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onNext\', \'SQ\', \''+this.VUID+'\')"><i>'+MultiArray["Next"][lang]+'';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		if(this.sql_query == "")
		{
			alert(MultiArray["Error: Missing SQL query for database request #"][lang]+this.uID);
		...
		else if(this.sql_query.trim().toLowerCase().indexOf("select") == 0)
		{
			if(this.res_name == "")		
			{
				alert(MultiArray["Error: Missing result name for database request #"][lang]+this.uID);

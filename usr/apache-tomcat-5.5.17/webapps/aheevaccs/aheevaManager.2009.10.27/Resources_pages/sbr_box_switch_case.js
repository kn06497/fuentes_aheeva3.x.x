...
function SwitchCase()
{
	...
	{			
		...
		sReturn += '<tr bgcolor="#E8E0DB"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDSwitchCase_'+this.uID+'\')"><b>'+MultiArray["Switch"][lang]+' #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'SC\', \''+this.VUID+'\')"><i>'+MultiArray["Entry point"][lang]+'</i></td></tr>';
		...
		sReturn += '<tr><td colspan="2" nowrap class="onsuccedTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onDEFAULT\', \'SC\', \''+this.VUID+'\')"><i>'+MultiArray["Default"][lang]+'';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		{
			alert(MultiArray["Error: Missing possible values for switch #"][lang]+this.uID);
...
/**
fonction active lors qu'on tente de configurer un switch box
*/
...
{	
	...
	var str = document.popUpForm.n_call_prop2[document.popUpForm.n_call_prop2.selectedIndex].value;
	switch (str){
		case ("WEEKDAY"):
			modifyTestWindow(vectorOfWeekDay, document.popUpForm.SCSetupDialog_SCValueParams, document.popUpForm.SCSetupDialog_options, document.popUpForm.SCSetupDialog_list_of_SCValues, document.popUpForm.SCSetupDialog_list_of_SCValues2);
			break;
		case ("DAYNAME"):
			modifyTestWindow(vectorOfDayName, document.popUpForm.SCSetupDialog_SCValueParams, document.popUpForm.SCSetupDialog_options, document.popUpForm.SCSetupDialog_list_of_SCValues, document.popUpForm.SCSetupDialog_list_of_SCValues2);
			break;
		case ("MONTHNAME"):
			modifyTestWindow(vectorOfMonthName, document.popUpForm.SCSetupDialog_SCValueParams, document.popUpForm.SCSetupDialog_options, document.popUpForm.SCSetupDialog_list_of_SCValues, document.popUpForm.SCSetupDialog_list_of_SCValues2);
			break;
		case ("DAYOFWEEK"):
			modifyTestWindow(vectorOfMonthNumber, document.popUpForm.SCSetupDialog_SCValueParams, document.popUpForm.SCSetupDialog_options, document.popUpForm.SCSetupDialog_list_of_SCValues2, document.popUpForm.SCSetupDialog_list_of_SCValues);
			break;
		case ("MONTHOFYEAR"):
			modifyTestWindow(vectorOfMonthNumber, document.popUpForm.SCSetupDialog_SCValueParams, document.popUpForm.SCSetupDialog_options, document.popUpForm.SCSetupDialog_list_of_SCValues2, document.popUpForm.SCSetupDialog_list_of_SCValues);
			break;
		default:
			document.popUpForm.SCSetupDialog_options.style.display = "none";
			document.popUpForm.SCSetupDialog_SCValueParams.style.display = "block";
			document.popUpForm.SCSetupDialog_list_of_SCValues2.style.display = "none";
			document.popUpForm.SCSetupDialog_list_of_SCValues.style.display = "block";
	}
}
...
{
	if((document.popUpForm.SCSetupDialog_list_of_SCValues.value != null && document.popUpForm.SCSetupDialog_list_of_SCValues.value != "") || (document.popUpForm.SCSetupDialog_list_of_SCValues2.value != null && document.popUpForm.SCSetupDialog_list_of_SCValues2.value != ""))
	{
		var theText;
		if(document.popUpForm.SCSetupDialog_list_of_SCValues2.style.display != "block"){
			if (document.popUpForm.SCSetupDialog_SCValueParams.value != "" && document.popUpForm.SCSetupDialog_SCValueParams.value != null){
				theText = document.popUpForm.SCSetupDialog_list_of_SCValues.value + ' '+ document.popUpForm.SCSetupDialog_SCValueParams.value;
				newop = new Option(theText, document.popUpForm.SCSetupDialog_list_of_SCValues.value);
			}
			else{
				theText = document.popUpForm.SCSetupDialog_list_of_SCValues.value + ' '+ document.popUpForm.SCSetupDialog_options[document.popUpForm.SCSetupDialog_options.selectedIndex].value;
				newop = new Option(theText, document.popUpForm.SCSetupDialog_list_of_SCValues.value);
			}
		}
		else{
			theText = document.popUpForm.SCSetupDialog_list_of_SCValues2.value + ' ' + document.popUpForm.SCSetupDialog_options[document.popUpForm.SCSetupDialog_options.selectedIndex].value;	
			newop = new Option(theText, document.popUpForm.SCSetupDialog_list_of_SCValues2.value);	
		}

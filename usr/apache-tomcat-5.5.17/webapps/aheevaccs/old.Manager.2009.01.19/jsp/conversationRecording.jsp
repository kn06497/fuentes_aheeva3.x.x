...
<script >
	...
	function submitInfo()
	{
		...
		//alert('servPath='+servPath + ' encparams=' +encparams + ' Protocol= '+proto + ' encServerHost=' +encServerHost + 'encServerPort='+encServerPort )
		var nextLoc ="./wavFileGenerator.jsp?record_id=";
		nextLoc +=record;
		nextLoc +="&path=";
		nextLoc +=pathval;
		nextLoc +="&agt=";
...
<jsp:useBean id="beanRecord" scope="request" class="Manager.reports.beanConversationRecord" />
	   ...
	   if(agents.compareTo("AllAgents") !=0)
		   {
			  ...
			  beanRecord.getConversationRecordsForAgent (agents,agentGrp,startingDate,
					endingDate, option, phoneNumber,option_qm,option_sales,option_no_sales);
		   }
	   else{
			  agtVar =  agentGrp;
			  agentReport = false;
			  beanRecord.getConversationRecordsForGroup (agentGrp,startingDate,
				   endingDate, option, phoneNumber,option_qm,option_sales,option_no_sales,limit, "", pagenum);
		   }
...
<p align="center">&nbsp; </p>
<div class="tab-page" >
  <fieldset > <h5><legend><script>document.write(MultiArray["Listening to agent's conversations"][lang]);</script></legend> </h5><!--<legend class="genericTitle"> <h5>-->
  ...
  <div class="reportLook" > 
    <table width="400" border="0" align="center"  >
      ...
      <tr> 
        <td> <div > <b> 
            <% if(phoneNumber.length()>0) out.write(" where Phone Number = " + phoneNumber);%>
            </b></div></td>
      </tr>
    </table>
    <p  class="reportsID"> 
    <div style="background-color:#EAEAEA; width: 1000; height:18 " ><b> 
	<table><tr> <td>
	  ...
	  <td width="350" align="center">
		<b><% int start = (pagenum-1)*Integer.parseInt(limit); int endp =start+Integer.parseInt(limit);  out.write(Integer.toString(start)+"-"+Integer.toString(endp)+" <script>document.write(MultiArray[\"of\"][lang]);</script> " +numberofrecords+" <script>document.write(MultiArray[\"records found\"][lang]);</script>." );	%></b>
	  </td>	
	</tr></table>	
      </b> </div></p>
    <table border="1"  cellpadding="0" cellspacing="1" style="border-collapse:separate; border-color:#DCDAD6"  width="1000" id="AutoNumber1">
      <tr class="reportsTab"> 
...
%>
      <tr <% if(i%2 == 0) out.write("class=conversationRowPair");
        ...
        <td  height="27" align="center" >&nbsp;
			<input type="button" value="<%=pageData.getWord("Play")%>" onClick="submitInfo('<%=rec_id%>', '<%=path%>', '<%=agt_id%>', '<%=file%>', '<%=cur_phoneNumber%>', '<%=encryption%>', '<%=grp_dbid%>', '<%=enc_method%>', '<%=enc_server_dbid%>')">
      ...
      <%
  	  ...
  	  out.write("<tr ><td colspan=10 align=\"center\" height=50><font face=\"Arial, Helvetica, sans-serif\" size=\"2\" color=\"#000000\"><b>&nbsp; <script>document.write(MultiArray[\"Error while generating report\"][lang]);</script>.. </b></font></td></tr>");
   }
   
   
  }//end if
  else{
	  out.write("<tr><td colspan=10 align=\"center\" height=50><font face=\"Arial, Helvetica, sans-serif\" size=\"2\" color=\"#000000\"><b>&nbsp; <script>document.write(MultiArray[\"No records available\"][lang]);</script> </b></font></td></tr>");
  }
%>
      <!-- traverse the array -->
				...
				if( numpages > 1 )
					{						
						for(np=1; np <= 10 && np <=numpages; np++)
							if(np != pagenum)
								out.write("<td width='25' onclick=\"getRequestedRecordings('"+startingDate+"','"+endingDate+"','"+phoneNumber+"','"+np+"','"+limit+"','"+Integer.toString(numpages)+"','false','"+agentGrp+"','"+agents+"','"+option_sales+"','"+option_no_sales+"','"+numberofrecords+"')\"><a style='font-size:13px; font-family:Tahoma; color:#000000; cursor: hand ' >"+np+"</a></td>");
							else
								out.write("<td width='25'><a style='font-size:13px; font-family:Tahoma; color:#0000ff ' >"+np+"</a></td>");
															
						if(np < numpages)
							out.write("<td width='25' onclick=\"getRequestedRecordings('"+startingDate+"','"+endingDate+"','"+phoneNumber+"','"+(pagenum - (pagenum%10) + 10)+"','"+limit+"','"+Integer.toString(numpages)+"','false','"+agentGrp+"','"+agents+"','"+option_sales+"','"+option_no_sales+"','"+numberofrecords+"')\"><a style='font-size:13px; font-family:Tahoma; color:#000000; cursor: hand ' > next </a></td>");
					...
					if(np < numpages)
						out.write("<td width='25' onclick=\"getRequestedRecordings('"+startingDate+"','"+endingDate+"','"+phoneNumber+"','"+(pagenum - (pagenum%10) + 10)+"','"+limit+"','"+Integer.toString(numpages)+"','false','"+agentGrp+"','"+agents+"','"+option_sales+"','"+option_no_sales+"','"+numberofrecords+"')\"><a style='font-size:13px; font-family:Tahoma; color:#000000 ' > next </a></td>");

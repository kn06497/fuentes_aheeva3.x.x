...
<%@page import ="Manager.wfm.beanLoginProdSummary"%>
		...
		if(elt_name.match("ack") == "ack")
		{
			...
			//{
				if(!ack.disabled && (!ack.checked || comments == ""))
				{
					...
					document.getElementById("msg_err"+id).innerHTML = +MultiArray["You must acknowledge and enter a comment before submitting"][lang]+.fontcolor("navy");
					valid = false;
				}
			//} 
		}/*
...
function insertRow(tbody_id, id, login, table_id)
{
	...
	selectStr += '<option value="training"> '+MultiArray["Training"][lang] +' </option>';
	selectStr += '<option value="workshop1"> '+MultiArray["Workshop"][lang] +' 1</option>';
	selectStr += '<option value="workshop2"> '+MultiArray["Workshop"][lang] +'  2</option>';
	selectStr += '<option value="integration"> '+MultiArray["Integration"][lang] +' </option>';
	selectStr += '<option value="production"> '+MultiArray["Production"][lang] +' </option>';
	selectStr += '</select>';
	cell.innerHTML = selectStr;
	cell.align = "center";
...
<%	}
	...
	if(published)
		out.println("<b><font color='red'><script language='javascript'>document.write(MultiArray[\"Data is published, modifications are disabled. Contact your supervisor if you need to unlock it\"][lang]);</script></font></b>");
%>
  ...
  <table id="tblProduction" border="1" cellpadding="0" cellspacing="0" bordercolor="#111111" style="border-collapse:collapse ">
	...
	for (j = 0; j < data.size(); j++)
	{
		...
		%>
			...
			<tr>
                             <td width="41" bgcolor="#FDFDE0" align="center"><font color="#000000"><b> <script>document.write(MultiArray["Total"][lang])</script> </b></font></td>						 
                            <td width="235" colspan="2" bgcolor="#FDFDE0" align="center"><%if(!published){%><script>document.write("<input name=\"addRow\" type=\"button\" value=\""+MultiArray["Add a new row"][lang]+"\" onClick=\"insertRow(<%=old_agt_id%>, '<%=old_id%>', '<%=old_login%>', 'tblProduction')\">")</script><%}else{%>&nbsp;<%}%></td>						     
        	...
        	<td width="100" align="center" valign="middle">&nbsp;
			  ...
			  <center>
				<select name="type<%=id%>" onChange="modified=true;" disabled>
				  <option value="training" <%=r1.training?"selected":""%>  <%=published?"disabled":""%>><script>document.write(MultiArray["Training"][lang])</script></option>
				  <option value="workshop1" <%=r1.workshop1?"selected":""%>  <%=published?"disabled":""%>><script>document.write(MultiArray["Workshop"][lang])</script> 1</option>
				  <option value="workshop2" <%=r1.workshop2?"selected":""%>  <%=published?"disabled":""%>><script>document.write(MultiArray["Workshop"][lang])</script> 2</option>
				  <option value="integration" <%=r1.integration?"selected":""%>  <%=published?"disabled":""%>><script>document.write(MultiArray["Integration"][lang])</script></option>
				  <option value="production" <%=r1.production?"selected":""%>  <%=published?"disabled":""%>><script>document.write(MultiArray["Production"][lang])</script></option>
				</select>
			  </center>
...
//login_total_break = (tLoginTime < 7200) ? tLoginTime : tLoginTime + 900 * ((tLoginTime / 14400) + 1);   // 14400 = 4 h.   1 pause de 15 min(900 s) par tranche de 4 heures
  ...
  %>
    ...
    <tr>
      <td width="41" height="23" align="center" bgcolor="#FDFDE0"><font color="#000000"><b> <script>document.write(MultiArray["Total"][lang])</script> </b></font></td>
      <td width="235" colspan="2" bgcolor="#FDFDE0" align="center">
        <%if(!published){%>
		<script>document.write("<input name=\"addRow\" type=\"button\" value=\""+MultiArray["Add a new row"][lang]+"\" onClick=\"insertRow(<%=r1.agt_id%>, '<%=id%>', '<%=login%>', 'tblProduction')\">")</script>
        <%}else{%>
        &nbsp;
        <%}%>
      </td>
...
<strong><script>document.write(MultiArray["Other training"][lang])</script></strong>
<table id="tblOtherTraining" border="1" cellpadding="0" cellspacing="0" bordercolor="#111111" style="border-collapse:collapse ">
	...
	<script>document.write("<input name=\"addTraining\" type=\"button\" value=\""+MultiArray["Add a new row"][lang]+"\" onClick=\"insertRowOther('other_training', 'tblOtherTraining')\">")</script>
</td></tr>
	<tr class="reportsTab">
	  ...
	  <td align="center" colspan="4"><b><script>document.write(MultiArray["Training type"][lang])</script></b></td>
...
{
%>
<h3><strong><script>document.write(MultiArray["Summary"][lang])</script></strong></h3>
<table width="50%" class="reportsTab" border="0" align="left">
	<tr>
		<td><strong><script>document.write(MultiArray["Name"][lang])</script></strong></td>
		<td><strong><script>document.write(MultiArray["Queue"][lang])</script></strong></td>
		<td><strong><script>document.write(MultiArray["Production time"][lang])</script></strong></td>
	</tr>
	<%
...
%>
      <div id="PasswordVerifDialog" style="visibility:hidden; border:2px solid #000000; background-color:#FFFFFF; width:200px;position:absolute; z-index:500; top:0px; left:0px; ">
            <table border="0" width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                            <td colspan="2" align="center" bgcolor="#6AA5D7"><b><script>document.write(MultiArray["Please enter your password"][lang])</script></b></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
					  ...
					  <script>document.write("<input type=\"button\" value=\""+MultiArray["Cancel"][lang]+"\" onClick=\"closePasswordDialog()\">")</script>

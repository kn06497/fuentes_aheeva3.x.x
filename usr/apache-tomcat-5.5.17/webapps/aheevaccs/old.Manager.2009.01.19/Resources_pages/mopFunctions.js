...
function chkAndAddTreatmentToMop()
{
	...
	else
	{
			...
			alert(MultiArray["You can select only one treatment for this mop !! Please drop the selected treatment and select another one"][lang]);
...
function displayMopInfo()
{
   ...
   // Remove all treatment from selectedTreatments option field
	...
	//alert(mop_name)
	freeTreatmentList = appletObj.getAvailableTreatmentsForMop(mop_name); 
	//freeTreatmentList = appletObj.getActiveTreatments();
	//alert (freeTreatmentList)
	...
	arrayTreatmentList = jsTreatmentLists.split("|");
   	...
   	for (i = 0; i <arrayTreatmentList.length; i++)
	{
	...
	if(mop_name == '' || mop_name == 'null' || mop_name == 'NULL' || mop_name == null)
	{
		...
		document.mop_form.updateInfo.value = MultiArray["Save"][lang];
...
function validateInfo()
{
	...
	alert(MultiArray["You must enter a Mop name"][lang]);
           
	else if(validation(document.mop_form.mopName.value)==0)
	{
...
{
	var t_name = document.mop_form.mopName.value;
	answer = confirm("Are you sure, you wish to delete this method of payment? \n (Please check the agent groups which uses this MOP. )")
	if(answer){

...
var w_monthname=new Array(""+MultiArray["January"][lang]+"",""+MultiArray["February"][lang]+"",""+MultiArray["March"][lang]+"",""+MultiArray["April"][lang]+"",""+MultiArray["May"][lang]+"",""+MultiArray["June"][lang]+"",""+MultiArray["July"][lang]+"",""+MultiArray["August"][lang]+"",""+MultiArray["September"][lang]+"",""+MultiArray["October"][lang]+"",""+MultiArray["November"][lang]+"",""+MultiArray["December"][lang]+"");
var w_dayname = new Array(""+MultiArray["Sunday"][lang]+"",""+MultiArray["Monday"][lang]+"",""+MultiArray["Tuesday"][lang]+"",""+MultiArray["Wednesday"][lang]+"",""+MultiArray["Thursday"][lang]+"",""+MultiArray["Friday"][lang]+"",""+MultiArray["Saturday"][lang]+"");
var w_StartOfWeek = 1; // Monday
var curweek_in_month = 0;
	...
	do 
	{
		...
		if (j == 6) 
		{
            ...
            if(window.addEventListener){ // Mozilla, Netscape, Firefox
                ...
                weekEl.attachEvent('onclick', w_SetWeekDateNew);
            }
            ///////
...
function w_SetWeekDate(evt)
{
    ...
    if(mMonth<10)
        ...
        m = mMonth
//alert(rowWeek+'..'+rowWeek+istartWeek+'..'+rowWeek+iendWeek)
...
function w_SetWeekDateNew(evt)
{
    ...
    mMonth = (w_d.getMonth()+1);
    var ms=mMonth, me=mMonth; 
    if(mMonth<10)
        m = "0" + mMonth
    else
        m = mMonth

    mDayE = document.getElementById("w_c"+rowWeek+'6').innerHTML;  	
    mDayS = document.getElementById("w_c"+rowWeek+'0').innerHTML;  	
    ...
    if(parseInt(mDayE,10) < parseInt(mDayS,10)){
	  if(rowWeek == 4){	
		me = (mMonth + 1 <= 12 ? mMonth+1 :12);	  	
	  }
	}
	me=(me<10 ? "0"+me : me);
    endW = me + "/" + g + "/" + w_d.getFullYear();
    
    mDayS = document.getElementById("w_c"+rowWeek+'0').innerHTML;  	
	//alert(mDay+'..g..'+parseInt(g,10))
    if( mDayS > parseInt(g,10)){
	  if(rowWeek == 0){	
		m = (mMonth - 1 > 0 ? mMonth-1 :12);	  	
		m=(m<10 ? "0"+m : m);
	  }
	}
	var yy = w_d.getFullYear();
	if(m != mMonth && m==12)
		yy = yy -1;
    ...
    try
    {
		...
		document.getElementById('WeeklyDateStart').value = startW;
		document.getElementById('WeeklyDateEnd').value  = endW;	
...
function w_displayCal()
{
	...
	{
		if(document.getElementById('weeklyCalendar') != null){
			document.getElementById('weeklyCalendar').style.left=window.event.x+document.body.scrollLeft;
			document.getElementById('weeklyCalendar').style.top=window.event.y+document.body.scrollTop;
    ...
    for (i = 0; i < 7; i++)
    {
        ...
        try
        {
            ...
            if(window.removeEventListener()){ // Mozilla, Netscape, Firefox
	            ...
	            week.detachEvent('onclick', w_SetWeekDateNew);
            }
        }
        catch(e){};
...
function w_writeDayname()
{
    ...
    document.write('<td class="week" id="weekHeader">'  +MultiArray["week"][lang] + '</td>');
...
function buildWeeklyCalendar(WeekStart)
{
    ...
    document.write('<br><table class="calendar" >');
    document.write('<tr><td colspan="8">');
    // header table
    document.write('<table width="100%" cellpading="0" cellspacing="0">');

    document.write('<tr class="firstrow"><td width="8px" onClick="w_renderCalendar(-1);" align="right" ><img src="../img/arrow_left_cal.gif" title="' + prev_month_title + '" border="0"></td>');
    document.write('<td width="8px" onClick="w_renderCalendar(1);" align="left" ><img src="../img/arrow_right_cal.gif" title="' + next_month_title + '" border="0"></td>');

...
<jsp:useBean id="beanPayrollDataObj" scope="request" class="aheevaManager.wfm.beanPayrollData" />
...
function validateDataToSubmit()
{
	...
	var m_year = temp.substring(6, 10);
	var m_month =temp.substring(0, 2) - 1;
	var m_day =temp.substring(3, 5);

	var start = new Date( m_year,m_month,m_day);

	//var end = Date(document.ceridian_report_form.end_date.value);

	if(start.getDay() != 0)
	{
		alert("<%= pagePayData.getWord("Report starting date must be Sunday!")%>");
		return false;
	}
	else
	{
	...
	m_year = temp.substring(6, 10);
	m_month =temp.substring(0, 2) - 1;
	m_day =temp.substring(3, 5);
	var end = new Date( m_year,m_month,m_day);
/*
	alert("end " + end)
	alert("Date "+ end.getDate())
	alert("Day "+ end.getDay())
	alert("Month"+ end.getMonth())
	alert("Year"+ end.getYear())
*/

	if(end.getDay() != 6)
	{
		alert("<%= pagePayData.getWord("Report ending date must be Saturday!")%>");
		return false;
	}
	else
	{
	...
	if (start > end)
	{
		alert("<%= pagePayData.getWord("Starting date must be prior to ending date")%>");
		return false;
	}

	if (end.getTime() - start.getTime() > 522000000)
	{
		alert("<%= pagePayData.getWord("You cannot extract data for more than one week")%>");
		return false;
	}

	if (document.ceridian_report_form.exportTo.value == '')
	{
		alert("<%= pagePayData.getWord("You must select a payroll company")%>");
		return false;
	}

	if (document.ceridian_report_form.exportEmail.value == '')
	{
		alert("<%= pagePayData.getWord("You must specify an email adress to which the report will be sent")%>");
		return false;
	}
	else
	{
	...
	document.ceridian_report_form.export_from.value = document.ceridian_report_form.exportFrom.value;

	
	var destination = "./extractPayrollData.jsp?start_date=";
	destination +=document.ceridian_report_form.date_start.value;
	destination +="&date_end=";
	destination +=document.ceridian_report_form.date_end.value;
	destination +="&pay_system=";
	destination +=document.ceridian_report_form.pay_system.value;
	destination +="&export_addr=";
	destination +=document.ceridian_report_form.export_addr.value;
	destination +="&export_from=";
	destination +=document.ceridian_report_form.export_from.value;	
  ...
  <div class="dynamic-tab-pane-control">
	<div class="tab-pane" id="tabPane1">
 		...
 		<div class="tab-page" id="general">
			<fieldset>
				<legend><%= pagePayData.getWord("Extract payroll data")%></legend>
					<form name="ceridian_report_form" target="_self" action="extractPayrollData.jsp" method="post" onKeyDown="KeyDownHandler(getCeridianData)">
						<table>
							<tr >
								<td>
								</td>
							...
							<tr>
								...
								<td valign="middle" class="style1">
									...
									&nbsp;&nbsp;&nbsp;&nbsp;<%= pagePayData.getWord("To")%>&nbsp;&nbsp;<input type="text" id="end_date" name="end_date" onFocus="showCalendar('',this,this,'','id_reportDate',0,10,1);this.blur()" size="15">
								</td>
							</tr>
							<tr >
								<td>
							...
							<tr >
								<td class="style1">
                                  <%= pagePayData.getWord("Mail from")%>
								</td>
								<td>
                                  <input type="text" size="35" name="exportFrom" id="exportFrom" value="finance@aheeva.com"/>
								</td>
							</tr>
							<tr >
								<td class="style1">
								<%= pagePayData.getWord("Mail to")%>
								</td>
								<td>
							...
							<tr >
								...
								<td>
									<input id="getCeridianData" name="getCeridianData" type="button" value="<%= pagePayData.getWord("Submit")%>" onClick="return validateDataToSubmit();"/>
								</td>
							</tr>
						</table>
						...
						<input type="hidden" id="export_from" name="export_from" />

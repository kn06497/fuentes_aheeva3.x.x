...
function getAgentsInGroup(groupBox, agentBox, viewBox)
{
	...
	if(groupBox.value =='AllGroups')
	{
		...
		agentBox.length = 0;
        var defaultvalue = '- <%= pageCorepData.getWord("Select an agent")%> -'; //  "- All Agents -" ;
...
function dateChecker()
{
	...
	{
		alert('<%= pageCorepData.getWord("Please enter a valid date to proceed")%>');
		return false;
	}	
	else if(endDate < startDate)
	{
		alert("<%= pageCorepData.getWord("Your ending date cannnot be smaller than your starting date")%>");
		return false;
	}
...
function choiceChecker()
{
	...
	{
		alert("<%= pageCorepData.getWord("Please select at least one object")%>");
		response = false;
	}
...
<body  class="rep_body" onLoad="loadAgentGrp()">
<%String sret ="";%>
...
<form name="agentHours"  action="./commissionReport.jsp" method="post"    >
	...
	<script type="text/javascript">
	  ...
	  <div class="tab-page" id="searchoptions">
	  	<h4 class="tab"><%= pageCorepData.getWord("Commissions")%></h4>

			<script type="text/javascript">
				tp1.addTabPage( document.getElementById( "searchoptions" ) );
			</script>
			<fieldset>
				<legend><%= pageCorepData.getWord("Search options")%></legend>
				<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width="1455">
					<tr>
						<td height="25" colspan="4">&nbsp;						</td>
					</tr>
					<tr>
						<td align="right" width="251" height="23" style="border-left-style: none; border-left-width: 1; border-top-style: none; border-top-width: 1">							<b><%= pageCorepData.getWord("Show commission details from")%></b>						</td>
						<td width="34" ></td>
					  ...
					  <input type="text" size="15" onFocus="showCalendar('',this,this,'','id_fromDate',310,95,1);this.blur()" name="fromDate" id="fromDate" value=""/>		</td>				
						<td width="948">&nbsp;<b><%= pageCorepData.getWord("to")%></b>
					...
					<tr>
						<td align="right" width="251" height="23" style="border-left-style: none; border-left-width: 1; border-top-style: none; border-top-width: 1">
							<b><%= pageCorepData.getWord("Select an agent group")%></b>
					  <input name="agtChoice" type="radio" value="agentGroup" onClick="changeDisabled();"></td>
						...
						<div id="listAgtGrp" style="visibility:hidden" >
							<select  id="agtGrp" name="agtGrp" onChange="getAgentsInGroup(this, document.agentHours.objects, document.agentHours.viewckbox);" class="contentText" >
								<option value="AllGroups">- <%= pageCorepData.getWord("Select a group")%> -</option>
							</select>		
					  </div>				</td>
						<td valign="middle" >&nbsp;</td>
					</tr>
					<tr>
						<td align="right" width="251" height="23" style="border-left-style: none; border-left-width: 1; border-top-style: none; border-top-width: 1">
							<b><%= pageCorepData.getWord("Select an agent")%></b>
						...
						<td width="34" ></td>
							...
							<td colspan="2" id="id_reportDate" style="border-top-style: none; border-top-width: 1">
								...
								<select  id="objects" name="objects" class="contentText" onChange="getAgents(this)" >
									<option value="AllAgents">- <%= pageCorepData.getWord("Select an agent")%> -</option>
								</select>	
								...
								<input type="checkbox" name="viewckbox" onClick="javascript:changeAgentListView(document.agentHours.agtGrp, document.agentHours.objects, this);" ID="Checkbox2">
              					 <b><%= pageCorepData.getWord("Show agent by ID")%></b>
								</div>
							</td>
					...
					<tr>
						...
						<td width="948">&nbsp;<input id="viewAgentsHours"  name="viewAgentsHours" type="button"  value="<%= pageCorepData.getWord("View data")%>" onClick="extractAllData()">
			...
			<!--Requete pour l'agent group-->
				<% if (allDetails.size() >0  && reqType.compareTo("1") == 0 ) {%>				
					...
					<b><script>document.write("<%=pageCorepData.getWord("Commission details")%> "); document.write("<%=pageCorepData.getWord("for")%> ")</script><%=((beanCommissionDetails)allDetails.elementAt(0)).agent_group_name%></b></td></tr>
					
					<tr>
					 <td  class="reportsID" width="20%" align="center">
					<b><%=fromDate %><%=pageCorepData.getWord("To")%> <%=toDate%></b>  </td>
					</tr>
			...
			//Requete pour un agent
				if(allDetails.size() >0  && reqType.compareTo("2") == 0 ){%>
					...
					<tr>
					<td  class="reportsID" width="20%" align="center">
					<b><script>document.write("<%=pageCorepData.getWord("Commission details")%> "); document.write("<%=pageCorepData.getWord("for")%> ")</script><%=((beanCommissionDetails)allDetails.elementAt(0)).agent_name%></b></td></tr>
			...
			<!--Debut affichage details pour l'objet choisi-->
				...
				<tr>
					...
					<td align="center" width="100"><b><%=pageCorepData.getWord("Agent ID")%></b></td>
					<td align="center" width="200"><b><%=pageCorepData.getWord("Last name")%>, <%=pageCorepData.getWord("First name")%></b></td>	<%}%>
					<% if (reqType.compareTo("2") == 0) { //pour un agent%>
					<td align="center" width="110"><b><%=pageCorepData.getWord("Group ID")%></b></td>
					<td align="center" width="150"><b><%=pageCorepData.getWord("Group name")%></b></td>
					<%}%>								
					<td align="center" width="100"><b><%=pageCorepData.getWord("Agent sales")%></b></td>
					<td align="center" width="100"><b><%=pageCorepData.getWord("Group sales")%></b></td>
					<td align="center" width="125"><b><%=pageCorepData.getWord("Agent hours")%></b></td>
					<td align="center" width="125"><b><%=pageCorepData.getWord("Group hours")%></b></td>
					<td align="center" width="125"><b><%=pageCorepData.getWord("Calculation base")%></b></td>
					<td align="center" width="130"><b><%=pageCorepData.getWord("Total commission")%></b></td>
					<td align="center" width="130"><b><%=pageCorepData.getWord("Commission per hour")%></b></td>
					<td align="center" width="200"><b><%=pageCorepData.getWord("Commission details")%></b></td>
			 	</tr>
			...
			<!--Affichage du total-->
					<tr>
					  <td bgcolor="#FFECF5" align="center"> <b><%=pageCorepData.getWord("Total")%></b></font></td>					  

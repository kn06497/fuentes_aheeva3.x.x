...
function setColumHeadersValue()
  {
	   ...
	   myData[0][0]=" <%=  pageadVData.getWord("Time hh:mm")%>";
	   hoverHelpData[0][0]="";
	   cells[0][0]= "";
	   personalData[0]="";
	   adherenceDetails[0]="";
	   ...
	   if(rows > 1 ){
		  ...
		  for(cnt = 0;cnt < allAgentScheduleData.length;cnt++){
									 ...
									 for(cpos=stPos; cpos <= ePos; cpos++){
									 	if(nElt == 3){     //we got schedule info
										   ...
										   //alert('..cellval..'+ cellval + '..cntRow..'+cntRow+'..cpos+1..'+cpos+1)
											...
											var oops0=" <%=  pageadVData.getWord("was LOGGED IN.** Should be logged in during this period")%>";
											var oops1="<%=  pageadVData.getWord("was LOGGED IN.** Should be logged in during this period")%>";
										   if(cellval == 0 || cellval == 4)
										   		{
													if(cellval == 0)
														hoverHelpData[cntRow+1][cpos+1] = curAgtID+ ' [\"+curAgtName+\"] '+ oops0.replace("**","\n");
													else
														hoverHelpData[cntRow+1][cpos+1] = curAgtID+ ' [\"+curAgtName+\"] '+ oops1.replace("**","\n");
	   	...
	   	viewHTML += '<TR>';
			    ...
			    viewHTML += "<TD align='justify' height=50   colspan="+cols+"><font face='Arial, Helvetica, sans-serif' size='3' color='#ff0000'><b> <marquee WIDTH=100% direction=right behavior=alternate><%= pageadVData.getWord("No schedule data found in database.")%></marquee></b></font></TD>";
...
<table width="947" border="0" align="center">
  <tr>
    ...
    <td width="350" valign="top" ><font size=4><b><%=pageadVData.getWord("Agent")%>	
        <% 
		...
		%>
		<%=pageadVData.getWord("Adherence report")%></b></font>
	<%
    ...
    <td width="438" valign="top"  class="reportsID"><table width="373" border="0">
      <tr>
        ...
        <td ><%=pageadVData.getWord("Work period")%></td>
        <td bgcolor="#339933" >&nbsp;</td>
        <td ><%=pageadVData.getWord("Adherence value")%>:<%=pageadVData.getWord("yes")%> </td>
      </tr>
      <tr>
        <td width="11" height="4" bgcolor="#FFFF00">&nbsp;</td>
        <td width="183" ><%=pageadVData.getWord("Break/Meal period")%> </td>
        <td width="11" bgcolor="#FF0000" >&nbsp;</td>
        <td width="150" ><%=pageadVData.getWord("Adherence value")%>:<%=pageadVData.getWord("no")%></td>
      </tr>
      <tr>
        <td height="4" bgcolor="#3399CC">&nbsp;</td>
        <td ><%=pageadVData.getWord("Gap between work and break")%></td>
        <td >&nbsp;</td>
        <td >&nbsp;</td>
      </tr> 
    </table></td>
  ...
  <tr align="center"> 
    <td  height=25 bgcolor="#FFFFFF">&nbsp;</td>
  	<td  valign=middle><div align="justify" class="reportsID"><b><%=pageadVData.getWord("Adherence")%>:  

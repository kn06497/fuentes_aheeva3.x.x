...
function askforMenuReload(context) 
{
	...
	var today = new Date();
	message = today+"<br>&nbsp;&nbsp; "+glob[0]+" "+context+". "+glob[1]+" &nbsp;&nbsp;";
	
	message += "<input name=\"\" type=\"button\" value='"+glob[2]+"' onClick=\"reloadRBottom()\" style=\"font-family:verdana; font-size:11px; background-color:#EFEFEF; border:1px solid #000000; width:50px \">&nbsp;&nbsp;";
	
	message += "<input name=\"\" type=\"button\" value='"+glob[3]+"'  onClick=\"dismissbox()\" style=\"font-family:verdana; font-size:11px; background-color:#EFEFEF; border:1px solid #000000; width:50px\">&nbsp;&nbsp;";
	message += "<input name=\"\" type=\"button\" value='"+glob[4]+"'  onClick=\"dismissbox()\" style=\"font-family:verdana; font-size:11px; background-color:#EFEFEF; border:1px solid #000000; width:50px\">";
	//window.frames['comm'].switchFrameSet(true, 6.05);
	window.top.switchFrameSet(true, 6.05);	
	startDropinMessage(message," "+glob[5]+" ");	
...
function rBottomPageInContext(context)
{
	...
	{
	if( rBottomPage.indexOf("treatment1.jsp") != -1 )
		bRet = true;
	}
	else if(context == "FILTERS")
	{
	if( rBottomPage.indexOf("filter1.jsp") != -1 )
		bRet = true;
	}
	else if(context == "CALLING LISTS")
	{
	if( rBottomPage.indexOf("callingList1.jsp") != -1 )
		bRet = true;
	}
	...
	{
	if( rBottomPage.indexOf("addSwitch1.jsp") != -1 || rBottomPage.indexOf("modifySwitch1.jsp") != -1)
		bRet = true;
...
function handleReceiveManagerAlertMessage(alertMessage)
{
	...
	crossMainObj.innerHTML = '<B>&nbsp;&nbsp;'+""+glob[6]+""+':</B> '+message;	
...
function popUpNoConnectionWarning(theCTI_IP)
{
	...
	if(count_nb == nb_wait)
		...
		message = now_date+"<br>&nbsp;&nbsp;"+glob[7]+"  "+theCTI_IP+" !!";
		window.top.switchFrameSet(true, 5.25); //
		startDropinMessage(message, " "+glob[8]+"");	
...
function popUpRetriveConnectionMessage(theCTI_IP)
{
	...
	message = now_date+"<br>&nbsp;&nbsp;"+glob[9]+"  aheevaccs@"+theCTI_IP+" !!";
	window.top.switchFrameSet(true, 5.25); //
	startDropinMessage(message, ""+glob[10]+"");	
	//alert(now_date+"\nCouldn't get connection from "+theCTI_IP);			
...
{	
	var rBottomPage = window.top.main.rbottom.location.href;
	if( rBottomPage.indexOf("architecture.jsp") != -1 )
	{
...
function callNetworkPageFunction()
{
	...
	else
	{
		...
		if(window.top.main && window.top.main.rbottom && 
			...
			window.top.main.rbottom.location.href.indexOf('architecture.jsp') != -1)
		{
			var params = '';
			var fcall = '';

...
//var chaine3 = ""; // contient les infos à updater après CloseTheDay
	...
	var close_really = confirm(VsPro[4]);
...
function addMyRow(groupBox,agtBox,viewBox)
{
	...
	if (maxrow == 1)
	{
		...
		code += "<td align='center' width='10%'><b>"+VsPro[5]+"</b></td>";
		code += "<td align='center' width='20%'><b>"+VsPro[6]+", "+VsPro[8]+"</b></td>";
		code += "<td align='center' width='15%'><b>"+VsPro[7]+"</b></td>";
		code += "<td align='center' width='20%'><b>"+VsPro[9]+"</b></td>";
		code += "<td align='center' width='9%'><b>"+VsPro[10]+" (hh:mm:ss)</b></td>";
		code += "<td align='center' width='20%'><b>"+VsPro[11]+"</b></td>";
		code += "</tr>";
		...
		code = "";
		code += "<tr id='t1'>";
		code += "<td align='center' width='6%'> <input type='button' onClick='javascript:deleteMyRow("+nrows+");' value='"+VsPro[12]+"'></td>";
		...
		code += "<option value=''>"+VsPro[13]+"</option>";
		code += "</select>";
		code += "</td>";
...
function showPaidTimeDetails(agent_id, agent_name, login_time, not_paid_nrr_time, over_paid_nrr_time, paid_time, state)
{
	...
	if (document.getElementById('bulleDiv') != null)
	{
		if(state==1){
			...
			code +="<TR> <TD NOWRAP WIDTH=20% ALIGN='left'></TD><TD NOWRAP WIDTH=50%><b>"+VsPro[16] +" :</b></TD><TD NOWRAP WIDTH=30%>  "+login_time+"</TD></TR>"
			code +="<TR> <TD NOWRAP WIDTH=20% ALIGN='left'></TD><TD NOWRAP WIDTH=50%><b>"+VsPro[17] +" :</b></TD><TD NOWRAP WIDTH=30%>-("+not_paid_nrr_time+")</TD></TR>"
			code +="<TR> <TD NOWRAP WIDTH=20% ALIGN='left'></TD><TD NOWRAP WIDTH=50%><b>"+VsPro[18] +" :</b><TD NOWRAP WIDTH=30%>-("+over_paid_nrr_time+")</TD></TD></TR>"
			code +="<TR> <TD NOWRAP WIDTH=20% ALIGN='left'></TD><TD NOWRAP WIDTH=50%><b>"+VsPro[19] +" :</b></TD><TD NOWRAP WIDTH=30%>= "+paid_time+"</TD></TR>"
			code +="</TABLE>";

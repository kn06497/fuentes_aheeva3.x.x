...
/*articleMenu.add(new WebFXMenuItem("Look &amp; Feel", "javascript:showArticleTab( \"looknfeel\" )"));*/
	...
	<div class="tab-page" id="tabGeneral">
		<h2 class="tab"><%= pageCreaData.getWord("General")%></h2>
		<script type="text/javascript">tp1.addTabPage( document.getElementById( "tabGeneral" ) );</script>
		<fieldset>
			<legend><%= pageCreaData.getWord("General settings")%></legend>
					<%
				if(request.getParameter("modify") != null && request.getParameter("modify").indexOf("'") == -1 && request.getParameter("modify").indexOf('"') == -1 &&
					...
					if(request.getParameter("cmp_id").indexOf("'") != -1 || request.getParameter("cmp_id").indexOf('"') != -1 || request.getParameter("cmp_id").length() == 0){//error in the campaign id
						...
						try{
							rs.next();
							%><%= pageCreaData.getWord("Showing properties for")%>: <%= rs.getString(1) %> <a href="cmp_commission_creator.jsp">[<%= pageCreaData.getWord("Switch")%>]</a></br>
							<%= pageCreaData.getWord("Only active objects for the selected campaign are displayed")%>.</br>
							<%
							out.println(getTableHeader(pageCreaData));
					...
					%>
					<%= pageCreaData.getWord("Choose a campaign")%>
					<table align="center" border="1" class="reportsTabName" style="border-collapse:collapse " bordercolor="#CCCCCC">
					<tr>
						<td></td>
						<td align="center"><strong><%= pageCreaData.getWord("Name")%></strong></td>
						<td align="center"><strong><%= pageCreaData.getWord("Description")%></strong></td>
						<td align="center"><strong><%= pageCreaData.getWord("Choose")%></strong></td>
					</tr>
					<%

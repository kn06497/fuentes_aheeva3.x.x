...
function agentPickUpMethod()
{
	...
	{	
		...
		sReturn += '<tr bgcolor="#C5D3EB"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDPickMethod_'+this.uID+'\')"><b>'+AgntP[0]+' #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'PM\', \''+this.VUID+'\')"><i>'+AgntP[1]+'</i></td></tr>';
		...
		sReturn += '<tr><td colspan="2" nowrap class="onsuccedTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onTimeOut\', \'PM\', \''+this.VUID+'\')"><i>'+AgntP[2]+'';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		if(this.havingClause == "")
			...
			{
			alert(AgntP[7]+this.uID+ AgntP[8]);
...
function isValideExpression(expr, uid)
{
	...
	{		
		...
		if(nbParen == -1)
		{
			...
			alert(AgntP[9]+uid+ AgntP[10].replace("**","\n")+subExpr+AgntP[12].replace("**","\n")+j);

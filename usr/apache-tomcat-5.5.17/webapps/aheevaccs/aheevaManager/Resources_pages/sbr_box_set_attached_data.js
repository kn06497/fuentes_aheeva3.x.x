...
function SetAttachedData()
{
	...
	this.buildHTML = function()
		{			
			...
			sReturn += '<tr bgcolor="#8fbc8f"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDSetAttachedData_'+this.uID+'\')"><b>'+setd[0]+' #'+this.uID;
			if(DEBUG)
			...
			sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'SAD\', \''+this.VUID+'\')"><i>'+setd[1]+'</i></td></tr>';
			sReturn += '<tr><td colspan="2"><input type="button" onClick="setUpSAD(\''+this.uID+'\')" ';
			sReturn += 'value="'+buttonValue+'" class="setupButton3"></td></tr>';
			sReturn += '<tr><td colspan="2" nowrap class="entryTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'OnNext\', \'SAD\', \''+this.VUID+'\')"><i>'+setd[2]+'';
			if(DEBUG)

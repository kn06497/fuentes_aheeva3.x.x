...
function PbxExec()
{
	...
	this.buildHTML = function()
		{			
			...
			sReturn += '<tr bgcolor="#C8D6D7"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDPbxExec_'+this.uID+'\')"><b>'+pbx[0]+' #'+this.uID;
			if(DEBUG)
			...
			sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'PE\', \''+this.VUID+'\')"><i>'+pbx[1]+'</i></td></tr>';
			sReturn += '<tr><td colspan="2"><input type="button" onClick="setUpPE(\''+this.uID+'\')" ';
			sReturn += 'value="'+buttonValue+'" class="setupButton3"></td></tr>';
			sReturn += '<tr><td colspan="2" nowrap class="entryTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'OnNext\', \'PE\', \''+this.VUID+'\')"><i>'+pbx[2]+'';
			if(DEBUG)

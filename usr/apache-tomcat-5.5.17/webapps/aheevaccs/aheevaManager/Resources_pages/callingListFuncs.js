...
{
	if(document.callingList.UnlimitCkB.checked)
	{
		document.callingList.MaxAttemptsTxt.disabled = true; //disable the Max attempts text box if the unlimited ckbox is checked. 
	...
	else
	{
		...
		max_attempts = document.callingList.MaxAttemptsTxt.value;
		if(max_attempts == "")
		{
			max_attempts = "10000";
		}
...
function displayCallingListInfo()
{
	...
	{		
	 ...
	 {		
		...
		var filters = String( appletObj.getFiltersByNameAndStatus(tenant_dbid, '1') );		
...
function AddSelectedFilter()
{
	...
	// Before going further, first remove the currently associated filter
	removeAssociatedFilter();
...
function saveCallingList(withoutFilter){
	...
	var state = '';
	max_attempts = document.callingList.MaxAttemptsTxt.value;
	if(document.callingList.state.checked)	state = 1;
	else state = 0;
   ...
   if(validation(document.callingList.name.value)==0) {
		...
		document.callingList.table_name.focus();
		return false;
   }  else if(validation(max_attempts)==0) {
   		alert(CAll[2]);
...
function askDelete()
{
	var answer = confirm(CAll[5]);
	if(answer)
	{
		/* make sure that this calling list is not associate with a running campaign */
		var isLoaded = String(appletObj.isCallingListLoaded(callingListDBID));		
		if(isLoaded == "1")
			alert(CAll[6]);
		else
		{
...
{	
	...
	{
		case "1":
			var listName = document.callingList.name.value;
			var call_list = document.callingList.table_name.value;
			if(!isAlphaNumeric(listName)){
				alert(CAll[8]);
			} else if(!isAlphaNumeric(call_list)){			
				alert(CAll[9]);
		...
		case "5": //validate the after queries mega form
			...
			foundone =   isValidDialingTimeSelected();
			if(!foundone) {
				alert("The selected dialing hours are not correct !");
...
function isValidDialingTimeSelected() {
	 var startObj   =  document.getElementById("dialing_from");  
         var startTime  =  parseInt(startObj.options[startObj.selectedIndex].value);
	 
         var endObj     =  document.getElementById("dialing_to");
         var endTime    =  parseInt(endObj.options[endObj.selectedIndex].value);
...
function buildHTMLFromTableFieldList()
{
	var sReturn = "";
	sReturn += '<table>';
	sReturn += '<tr><td style="background:	ThreeDFace;" align="center">&nbsp;C&nbsp;</td><td style="background:ThreeDFace;">'+CAll[22]+'</td></tr>';								
	var color = '#FFFFFF';
	for(var i = 0; i < tableFieldsVector.length; i++)
	{
...
function saveExtraData(){
	...
	var countExtraData   = (document.callingList.countExtraData.value > 0) ? document.callingList.countExtraData.value :  0;

	var data  =  "";
	var name =  "";
	for(var i= 0 ;   i < countExtraData;  i++) {
		data  =    document.callingList[i].value;
		name  =    document.callingList[i].name;
		//alert("name   == " + name +   "  value == "  +  data  )	;
	
	}
}


function handleModifyingExtaDataType(type) {
  var selectedTypeObj     =    document.getElementById("modifyScreenPopOption");
  var selectedValue       =    type.value;

  var allExtraDataFieldObj=    document.getElementById("allextradata");
  var newExtraDatasObj    =    document.getElementsByName("newExtraDatas"); 
  var fieldName  	  =  ""; 	 	

   if(selectedValue == 'all') {
	allExtraDataFieldObj.disabled  = false;
//	allExtraDataFieldObj.value = '';

	for(var i=0; i< newExtraDatasObj.length;   i++){
		fieldName     =  "newExtraDatas"+i;
		var extraObj  =  document.getElementById(fieldName);
  ...
  if(selectedValue == 'each' ) {
		allExtraDataFieldObj.disabled  =  true;
		allExtraDataFieldObj.value = '';

		for(var i=0; i< newExtraDatasObj.length;   i++){
		    fieldName     =  "newExtraDatas"+i;
                    var extraObj  =  document.getElementById(fieldName);
		    extraObj.disabled = false;
		}
  }

...
function startParser(xml_data) {
    ...
    ************************************************************************************/
	...
	//var ixml = '<DATA src="ahphone"><LOGOUT enable="1" /><READY enable="1" /><NOT_READY enable="0" /><HANGUP enable="0" /><DIAL enable="1" /><CONFERENCE enable="0" /><TRANSFER enable="0" /><CLOSE_PHONE enable="0" /><HOLD enable="0" /><UNHOLD enable="0" /><NOT_READY_REASON enable="1" /></DATA>';
   // alert(xml_data);
    ...
    for (arr_i=0;arr_i<arr.length;arr_i++){
		//alert(arr[i]);
		parent.ah_decode_message(arr[arr_i], arr_i , arr.length - 1 );
	}

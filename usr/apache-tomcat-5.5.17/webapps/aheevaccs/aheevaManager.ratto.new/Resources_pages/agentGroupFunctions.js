...
function displayAgentGroupInfo()
{
	...
	if(groupName == '')
	{
		if(userHasRight("ALLOW_FINANCIAL_CONFIGURATION"))
		{
			...
			document.agentGroup.nonConfiguredAgents.disabled = true;
			document.agentGroup.updateInfo.value = MultiArray["Save"][lang];
...
function validateCallPrefix(){
	if (10 < document.agentGroup.callPrefix.value.length){
		alert(MultiArray["AGERR Length"][lang]);
		return -1;
	}
	...
	for (var i = 0; i < document.agentGroup.callPrefix.value.length; i ++){
		...
		alert(MultiArray["AGERR Char"][lang]);
		return -1;
	}
return 0;
}
...
function validateInfo()
{
	...
	if (document.agentGroup.grpName.value =='')	
		...
		alert(MultiArray["Invalid use of character"][lang] + ': ' + MultiArray["Group name"][lang]);
		...
		alert(MultiArray["Invalid use of character"][lang] + ': ' + MultiArray["Queue name"][lang]);	
...
function addMyNotGlobalRow()
{
	...
	{
		alert(MultiArray["There is no active non-global not-ready-reason in the system."][lang]);
	}
   	//alert(code)
   	lyr.innerHTML = code;
  	document.getElementById("NRR_items").appendChild(lyr);

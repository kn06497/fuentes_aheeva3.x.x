...
function GetStatistic()
{
	...
	{	
		...
		sReturn += '<tr bgcolor="#D1D8C2"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDGetStatistic_'+this.uID+'\')"><b>'+MultiArray["Get statistic"][lang]+' #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'GS\', \''+this.VUID+'\')"><i>'+MultiArray["Entry point"][lang]+'</i></td></tr>';
		sReturn += '<tr><td colspan="2"><input type="button" onClick="setUpGS(\''+this.uID+'\', \''+this.statType+'\', \''+this.data+'\', \''+this.variableName+'\')" ';
		sReturn += 'value="'+buttonValue+'" class="setupButton"></td></tr>';
		sReturn += '<tr><td colspan="2" nowrap class="onsuccedTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onNext\', \'GS\', \''+this.VUID+'\')"><i>'+MultiArray["Next"][lang]+'';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		{
			alert(MultiArray["Error: Missing statistic type for get statistic box #"][lang]+this.uID);
			
			retrieveTmpMessage();
			bReturn = false;
			setUpGS(this.uID, this.statType, this.data, this.variableName);			
		} else if(this.data == null || this.data == "")
		{
			alert(MultiArray["Error: Missing statistic data for get statistic box #"][lang]+this.uID);
		...
		{
			alert(MultiArray["Error: Missing statistic variable name for get statistic box #"][lang]+this.uID);
	...
	this.softvalidate = function()
	{
		...
		var tmpmess;
		var tmptext = "";
		var tmponclick = 'setUpGS(\''+this.uID+'\', \''+this.statType+'\', \''+this.data+'\', \''+this.variableName+'\')';	
...
function changeGSSetupInterface(vl)
{
	...
	if(vl == 'AgentGroup=') //si on a l'option group de selectionner, 
		...
		inHTML += '<option>-- '+MultiArray["Agent Groups"][lang]+' --</option>';
...
function setUpGS(p_uID, p_statType, p_data, p_storevariable)
{
	//Hide all others...
	hideAllToolsOptions();
	...
	if( obj )
	{
		...
		switch( curvalue )
		{
			...
			case "QUEUE_AVG_WAITING_TIME":
				text = MultiArray["Queue name"][lang];
			...
			case "GROUP_BUSY_FACTOR_GROUP":
				...
				text = MultiArray["Agent properties"][lang];
				var obj2 = document.getElementById('GSSetupDialog_operators_td');				
				if( obj2 )
				{

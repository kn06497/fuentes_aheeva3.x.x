...
String source_file_name = "";
  if (MultipartFormDataRequest.isMultipartFormData(request))
  {
	  ...
	  {
	  %>
	  	<script> alert(MultiArray["Please choose a file"][lang]);</script>
	  ...
	  else
	  {
	  	...
	  	{	  	
	 %>
	   		<script> alert(MultiArray["Invalid input file"][lang]);</script>
	 <%
	 		upload = false;
	  	}
     }//histVect.size()>0
  ...
  try{
	...
	//get tenant info list
  	ResultSet groupRs = pageData.executeFromString(getGrouprQry);
  	while(groupRs.next())
	{
...
tp1 = new WebFXTabPane( document.getElementById( "tabPane1" ) );
	...
	<div class="tab-page" id="tabPage1">
		<h2 class="tab"><script>document.write(MultiArray["Import"][lang])</script></h2>
		<script type="text/javascript">tp1.addTabPage( document.getElementById( "tabPage1" ) );</script>
		<fieldset>
			<legend><script>document.write(MultiArray["load schedule Wizard"][lang])</script></legend>
				<table>
                  <% if(msg != null && msg.compareTo("1")== 0 ){ %>
                  <tr>&nbsp;&nbsp; </tr>
				  <tr> <td align="left" style="color:blue" ; font-weight:bold> <script>document.write(MultiArray["Schedule Submited"][lang])</script>   </td></tr>
				  <%}%>
				  <% if(msg != null && msg.compareTo("0")== 0 ){ %>
				  <tr>&nbsp;&nbsp; </tr>
                  <tr> <td align="left" style="color:red" ; font-weight:bold > <script>document.write(MultiArray["Error occured while submitting schedule"][lang])</script>   </td></tr>
				  <%}%>
				  
				  <tr>
				  <td>
				   <script>document.write(MultiArray["Please upload a file with csv format (*.csv)"][lang])</script><br>
				 </td>
				 </tr>
				 <tr>
				  <td>
				    <script>document.write(MultiArray["Source File Name:"][lang])</script>
								...
								<tr><td><script>document.write(MultiArray["Agent group"][lang])</script>
                                &nbsp;&nbsp;
            	...
            	<% } %>
						...
						<tr>
						 <td>
                                                 <% if(upload==false ){ %>
                                                 <input type="submit" name="loadButton" value="<%=pageData.getWord("upload")%>" onClick="return upload();">
                                                 <%}else{%>
						 <input type="submit" name="submitButton" value="<%=pageData.getWord("submit")%>"  onClick="return getAction();">
                                                 <%}%>
						 </td>
						</tr>
					</table>

	...
	if(host_ip && host_port)
	{
		...
		if(phoneAppletObj)
		{
			...
			if( !isagentspy_alreadyexist( curAgtLogin ) && state!= "in_call.gif")
			{
				...
				displayErrormessage(MultiArray["You are already spying agent #"][lang]+curAgtLogin+'.');
		}
	}
	else
	{
		changeSpyTicker(MultiArray["Unable to find agent switch!"][lang]);
	}
}
...
function spy_stop()
{
	...
	if( phoneAppletObj )
	{
		...
		if( curcallid != -1 )
		{
			...
			displayErrormessage(MultiArray["Error: Unable to get call and hangup!"][lang]);
	}
}
...
function removeLiveListening(id)
{
	...
	MultiArray["Error: Unable to get call and hangup!"][lang]
		displayErrormessage(MultiArray["Error: Unable to remove call "][lang]+id);
}

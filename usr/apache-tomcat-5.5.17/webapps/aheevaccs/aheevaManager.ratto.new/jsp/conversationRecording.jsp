...
<script >
	...
	{	
		...
		var get_min = getRequestedRecordings.arguments[14];
                var get_max = getRequestedRecordings.arguments[15];
                var get_call_length= getRequestedRecordings.arguments[16];
...
<%
    ...
    String option_video = request.getParameter("video");
        ...
        String min = (request.getParameter("min"));
               int  min_recording_length =(min == null ? 1 : Integer.parseInt(min));
      String max = (request.getParameter("max"));
               int  max_recording_length =(max == null ? 1 : Integer.parseInt(max));   

         String option_call_length = request.getParameter("call_length");
                option_call_length =(option_call_length == null ? "" : option_call_length);
   ...
   beanConversationDetails r1=null;
    try{
	   if(agents.compareTo("AllAgents") !=0)
		...
		endingDate, option, phoneNumber,option_qm,option_sales,option_no_sales,limit, "", pagenum, option_audio, option_video, min_recording_length , max_recording_length , option_call_length);
		   }
	   else{
			  agtVar =  agentGrp;
			  agentReport = false;
			...
			if(firstload.compareTo("true")==0){
				...
				{						
					for(np=1; np <= 10 && np <=numpages; np++)
						if(np != pagenum)
							out.write("<td width='25' onclick=\"getRequestedRecordings('"+startingDate+"','"+endingDate+"','"+phoneNumber+"','"+np+"','"+limit+"','"+Integer.toString(numpages)+"','false','"+agentGrp+"','"+agents+"','"+option_sales+"','"+option_no_sales+"','"+numberofrecords+"','"+option_audio+"','"+option_video+"','"+min_recording_length+"','"+max_recording_length+"','"+option_call_length+"')\"><a style='font-size:13px; font-family:Tahoma; color:#000000; cursor: hand ' >"+np+"</a></td>");
						else
					...
					if(np < numpages)
						out.write("<td width='25' onclick=\"getRequestedRecordings('"+startingDate+"','"+endingDate+"','"+phoneNumber+"','"+(pagenum - (pagenum%10) + 10)+"','"+limit+"','"+Integer.toString(numpages)+"','false','"+agentGrp+"','"+agents+"','"+option_sales+"','"+option_no_sales+"','"+numberofrecords+"','"+option_audio+"','"+option_video+"','"+min_recording_length+"','"+max_recording_length+"','"+option_call_length+"')\"><a style='font-size:13px; font-family:Tahoma; color:#000000 ' > next </a></td>");

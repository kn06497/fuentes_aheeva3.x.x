	...
	else
	{
		...
		{ // The record is not found on the server..
			...
			else
			{
				...
				errormsg_end += pageData.getWord("Check also that the mount service is working properly");
...
<%
	...
	{ //The record is encrypted, is_encrypted=1
		...
		{// || serverHost.length() < 7 || serverSecurePort.length() < 1) { // min IP: 0.0.0.1, len=7
			%>
			<script >
				alert(MultiArray["Invalid Encryption Server Host IP or Port ! Please, check Encryption Server settings.."][lang]);
		...
		else if(servletUrl.length() < 1 )
		{
			%>
			<script >
				alert(MultiArray["Invalid Servlet URL ! Please, check Encryption Server settings.."][lang]);
				submitInfo('<%=mount_dir+filePath+record_filename%>', '<%=Boolean.toString(found)%>','<%=userDbid%>','<%=rec_id%>','<%=is_encrypted%>','');
			</script>
			<%
		}

...
function CustomServer()
{
	...
	this.buildHTML = function()
	{
		...
		sReturn += '<tr bgcolor="#F0D7CA"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDCustomServer_'+this.uID+'\')"><b>Custom Server #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'CS\', \''+this.VUID+'\')"><i>Entry Point</i></td></tr>';
		...
		sReturn += '';
		sReturn += '<tr><td colspan="2" nowrap class="onsuccedTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onNext\', \'CS\', \''+this.VUID+'\')"><i>Next';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		if(this.module == "")
		{
			alert('Error: Missing module name for Custom Server #'+this.uID);
			retrieveTmpMessage();
			bReturn = false;
		...
		{
			alert('Error: Missing function name for Custom Server #'+this.uID);
			retrieveTmpMessage();
			bReturn = false;
			setUpCS(this.uID, this.module, this.function_name, this.params);
		}			

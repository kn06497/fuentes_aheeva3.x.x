...
function GetDtmf()
{
	...
	this.buildHTML = function()
	{
		...
		sReturn += '<tr bgcolor="#CCCCCC"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDGetDtfm_'+this.uID+'\')"><b>Get Dtmf #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'DT\', \''+this.VUID+'\')"><i>Entry Point</i></td></tr>';
		...
		sReturn += '<tr><td colspan="2" nowrap class="entryTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'OnNext\', \'DT\', \''+this.VUID+'\')"><i>Output';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		{
			alert('Error: Missing variable name for GetDtmf #'+this.uID);
			retrieveTmpMessage();
			bReturn = false;
		...
		{
			alert('Error: Missing timeout value for GetDtmf #'+this.uID);
			retrieveTmpMessage();
			bReturn = false;
			setUpDT(this.uID, this.variable_name, this.maxlength, this.defaultvalue, this.timeout, this.voicemessage, this.enders);
		}

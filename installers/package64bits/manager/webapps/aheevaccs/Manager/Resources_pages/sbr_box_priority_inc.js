...
function PriorityInc()
	{
		...
		this.buildHTML = function()
		{
			...
			sReturn += '<tr bgcolor="#DCEBC7"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDPriority_'+this.uID+'\')"><b>Priority #'+this.uID;
			if(DEBUG)
			...
			sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'PI\', \''+this.VUID+'\')"><i>Entry Point</i></td></tr>';
			...
			sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'OnNext\', \'PI\', \''+this.VUID+'\')"><i>Output</i></td></tr>';
			sReturn += '</table>';
			sReturn += '</div>';
		...
		this.validate = function()
		{
			...
			if(this.Increment == "")
			{
				alert('Error: Missing increment for Call Increment #'+this.uID);
				retrieveTmpMessage();
				bReturn = false;

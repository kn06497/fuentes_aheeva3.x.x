...
function SqlQuery()
{
	...
	{	
		...
		sReturn += '<tr bgcolor="#DECAB6"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDSqlQuery_'+this.uID+'\')"><b>Database Query #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'SQ\', \''+this.VUID+'\')"><i>Entry Point</i></td></tr>';
		sReturn += '<tr><td colspan="2"><input type="button" onClick="setUpSQ(\''+this.uID+'\')" ';
		sReturn += 'value="'+buttonValue+'" class="setupButton"></td></tr>';
		sReturn += '<tr><td colspan="2" nowrap class="onsuccedTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onNext\', \'SQ\', \''+this.VUID+'\')"><i>Next';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		if(this.sql_query == "")
		{
			alert('Error: Missing SQL Query for Database Request #'+this.uID);
			retrieveTmpMessage();
			bReturn = false;
			setUpSQ(this.uID);
		}
		else if(this.sql_query.trim().toLowerCase().indexOf("select") == 0)
		{
			if(this.res_name == "")		
			{
				alert('Error: Missing result name for Database Request #'+this.uID);
				retrieveTmpMessage();
				bReturn = false;
				setUpSQ(this.uID);
			}

// JavaScript Document
var curAgtName = "", curAgtLogin = "", curAgtTenant = "", curAgtIP = "", curAgtHost = "", curAgtAst = "";
...
function setCurrentAgentGlobals(agtName, agtLogin, agtTenant, agtIP, agtHost, agtAst, agtPhoneVersion)
{
	...
	ah_MenuSeeAgtInfos_1.items["menu_AgtSeeInfosHost"].setDisplayText("Host ["+curAgtHost+"]");
	ah_MenuSeeAgtInfos_1.items["menu_AgtSeeInfosAsterisk"].setDisplayText("Switch ["+curAgtAst+"]");
	ah_MenuSeeAgtInfos_1.items["menu_AgtSeeInfosStarPhone"].setDisplayText("Phone ["+curAgtPhoneVersion+"]");
...
{
	var yesno = confirm('Notice:\nYou are about to force all agents in group '+curGrpName+' status to '+newStatus+'\nDo you want to continue ?');	
	if(yesno)
...
{
	var newUrl = "./agent.htm?agtName="+curAgtName+"["+curAgtLogin+"]&tenant_dbid="+curAgtTenant;
...
{
	var newUrl = './agentGroup.htm?grpName='+curGrpName+'&tenant_dbid='+curGrpTenant;
	var downWindow = window.top.main.rbottom;
	if(downWindow)
	{
		downWindow.location.href = newUrl;

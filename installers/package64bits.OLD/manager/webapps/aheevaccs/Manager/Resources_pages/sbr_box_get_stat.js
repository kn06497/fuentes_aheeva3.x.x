...
function GetStatistic()
{
	...
	{	
		...
		sReturn += '<tr bgcolor="#D1D8C2"><td nowrap width="90%" style="cursor:move;" onMouseDown="dragThis(\'IDGetStatistic_'+this.uID+'\')"><b>Get Statistic #'+this.uID;
		if(DEBUG)
		...
		sReturn += '<tr><td colspan="2" class="entryTD" onClick="setUpPotentialEndLnk(\''+this.uID+'\', \'GS\', \''+this.VUID+'\')"><i>Entry Point</i></td></tr>';
		...
		sReturn += '<tr><td colspan="2" nowrap class="onsuccedTD" onClick="setUpPotentialStartLnk(\''+this.uID+'\', \'onNext\', \'GS\', \''+this.VUID+'\')"><i>Next';
		if(DEBUG)
	...
	this.validate = function()
	{
		...
		{
			alert('Error: Missing statistic type for Get Statistic Box #'+this.uID);
			retrieveTmpMessage();
			bReturn = false;
			setUpGS(this.uID, this.statType, this.data, this.variableName);			
		} else if(this.data == null || this.data == "")
		{
			alert('Error: Missing statistic data for Get Statistic Box #'+this.uID);
			retrieveTmpMessage();
			bReturn = false;
			setUpGS(this.uID, this.statType, this.data, this.variableName);	
		} else if(this.variableName == null || this.variableName == "")
		{
			alert('Error: Missing statistic variable name for Get Statistic Box #'+this.uID);
			retrieveTmpMessage();
			bReturn = false;
			setUpGS(this.uID, this.statType, this.data, this.variableName);	
		}
